...
//             .where((student) => student.course.toLowerCase() == 'mca' && student.sem == sem)
//             .toList();
//       }
//
  ...
  Future<void> onModelReady(
    ...
    try {
      ...
      if (attendanceData.isNotEmpty) {
        ...
        var data = firstElement['attendance'];

        if (data!=null) {
for(var i in data){
  _attendanceRecords.add(AttendanceModel.fromMap(i));
}
        // _attendanceRecords = data
        //     .where((dynamic e) => e is Map<String, dynamic>)
        //     .map((e) => AttendanceModel.fromMap(e as Map<String, dynamic>))
      ...
      showException(
        error: e,
        retryMethod: () => onModelReady(date: date, sem: sem, course: course),
      );
    }
  }
  Future<void> addAttendance({required List<String> absentStudentIds}) async {
