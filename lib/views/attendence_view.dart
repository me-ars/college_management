...
void _openAttendanceDialog(BuildContext context, AttendanceViewModel model) {
  selectedUserIds.clear(); // Clear previous selection
                    ...
                    ListTile(
                      title: Text(
                        selectedDate == null
                            ...
                            : DateFormat("yyyy-MM-dd").format(selectedDate!),
                        style:
                            const TextStyle(color: AppPalette.primaryTextColor),
                      ),
                      trailing: const Icon(Icons.calendar_today,
                          color: AppPalette.primaryTextColor),
                      onTap: () async {
                    ...
                    ConstrainedBox(
                      ...
                      child: Autocomplete<String>(
                        optionsBuilder: (TextEditingValue textEditingValue) {
                          return model.allStudents
                              .map((s) => s
                                  .studentId) // Get student IDs from allStudents
                              .where((id) =>
                                  id.contains(textEditingValue.text) &&
                                  !selectedUserIds.contains(id))
                              .toList();
  ...
  Widget build(BuildContext context) {
    ...
    return BaseView<AttendanceViewModel>(
        onModelReady: (AttendanceViewModel model) {
          _semFilterController.text = '1';
          model.onModelReady(
              course: context.read<AppState>().faculty!.course,
              date:
                ...
                child: Column(
                  children: [
                    ...
                    model.viewState == ViewState.ideal
                      ...
                      itemBuilder: (context, index) {
                        final student = model.allStudents[index];
                        final isAbsent = model.absentStudentIds.contains(student.studentId);
